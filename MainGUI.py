...
class App(QMainWindow):
    ...
    def login_page(self):
        ...
        logo = self.draw_label('Employee Insight System', width=1200, font_size=50)
    ...
    def voice_search(self, search_textbox):
        ...
        try:
            with sr.Microphone() as source:
                ...
                search_textbox.setText(text)
                self.search_query(
                    text,
                    [self.id_radio_button, self.name_radio_button, self.last_name_radio_button]
                )
            ...
            self.display_popup("Error", "Could not understand your voice")

        except sr.RequestError:
            self.display_popup("Error", "Internet required for voice recognition")
    ...
    def search_page(self):
        ...
        search_textbox.setStyleSheet(
            ...
            % (self.colors['search_textbox_border_color'])
        )
        ...
        self.id_radio_button = id_radio_button
        self.name_radio_button = name_radio_button
        self.last_name_radio_button = last_name_radio_button

        # Buttons
        search_btn = self.draw_button(
            btn_text='Search',
            tool_tip_text='Search the record',
            icon_path='search.png'
        )

        voice_btn = self.draw_button(
            btn_text='Voice Search',
            tool_tip_text='Search using microphone',
            icon_path='mic.png'
        )

        back_btn = self.draw_button(
            btn_text='Back',
            tool_tip_text='Back to Home',
            icon_path='back.png'
        )
        ...
        search_btn.clicked.connect(
            lambda: self.search_query(
                search_textbox.text(),
                [id_radio_button, name_radio_button, last_name_radio_button]
            )
        )

        voice_btn.clicked.connect(
            lambda: self.voice_search(search_textbox)
        )
        ...
        self.layout.addWidget(voice_btn, 4, 1)

        self.layout.addWidget(widgets['searched_table'][0], 5, 0, 1, 2)
    ...
    def add_employee_page(self):
        ...
        pos_field = QComboBox()
        name_field.textChanged.connect(self.mark_form_changed)
        lname_field.textChanged.connect(self.mark_form_changed)
        age_field.textChanged.connect(self.mark_form_changed)
        salary_field.textChanged.connect(self.mark_form_changed)    
        ...
        pos_field.currentTextChanged.connect(self.mark_form_changed)
